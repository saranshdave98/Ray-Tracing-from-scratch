...
{
	pyr3 = c + Vec3((((l / 2)*cos(rot)) - ((sqrt(3)*l / 2)*sin(rot))), 0, ((-(sqrt(3)*l / 2)*cos(rot)) - (l*sin(rot) / 2)));
	return pyr3;
}

Vec3 Make_Pyramid4(Vec3 c, double l, double rot)
{
	pyr4 = c + Vec3((((l / 2)*cos(rot)) - ((l / (2 * sqrt(3)))*sin(rot))), (l*sqrt(2) / sqrt(3)), (((-l / (2 * sqrt(3)))*cos(rot)) - (l*sin(rot) / 2)));
	return pyr4;
}
...
struct Pyramid {
	...
	Vec3 getNormal() { return cross((p2 - p1), (p3 - p1)); }
	bool intersect(Ray ray, double t) {
		Vec3 o = ray.o;
		Vec3 d = ray.d;
		Vec3 n = getNormal();
		...
		Vec3 p = o + d*t;
		t_v = t;
		if (t_v > 0 && ((dot((p - ((p1 + p2) / 2)), (c - ((p1 + p2) / 2))) > 0) && (dot((p - ((p2 + p3) / 2)), (c - ((p2 + p3) / 2))) > 0) && (dot((p - ((p3 + p1) / 2)), (c - ((p3 + p1) / 2))) > 0)))
		{
			return true;
		}
		else
...
Sphere sphere(Vec3(350, 100, -200), 100);
...
Vec3 pyr_origin(75, 0, -25);
double pyr_len = 200;
double pyr_rotation = 3.1415 / 4;
...
Vec3 rayTrace(Ray ray)
{
	...
	if (c_wall2.intersect(ray, t)) {
		arr[10] = c_wall2.t_val();
		flag = 1;
	}
	if (c_wall3.intersect(ray, t)) {
		arr[11] = c_wall3.t_val();
		flag = 1;
	}
	if (c_wall4.intersect(ray, t)) {
		arr[12] = c_wall4.t_val();
		flag = 1;
	}
	if (c_ceiling.intersect(ray, t)) {
		arr[13] = c_ceiling.t_val();
		flag = 1;
	}
	...
	if (min_ind == 0)
	{
		...
		if (dt > 0)
			return rayTrace(ray2)*1 + red*(0.5+0.5*dt)*0;
		else
			return rayTrace(ray2)*1 + red*(0.5)*0;
	}
	if (min_ind == 1)
	{
	...
	if (min_ind == 4)
	{
		...
		if (sphere.intersect(ray_dummy, t_dummy) || pyr_wall1.intersect(ray_dummy, t_dummy) || pyr_wall2.intersect(ray_dummy, t_dummy) || pyr_wall3.intersect(ray_dummy, t_dummy) || c_wall1.intersect(ray_dummy, t_dummy) || c_wall2.intersect(ray_dummy, t_dummy) || c_wall3.intersect(ray_dummy, t_dummy) || c_wall4.intersect(ray_dummy, t_dummy) || c_ceiling.intersect(ray_dummy, t_dummy))
		{
			return yellow*(0.5 + 0.5*dt)*0.2;
		}
		else
		{
			return yellow*0.5*(0.5 + 0.5*dt);
		}
	}
	if (min_ind == 5)
	{
...
int main() {
	...
	for (int y = Height-1; y >= 0; --y) {
		for (int x = 0; x < Width; ++x) {
			Ray ray(Vec3(x, y, 0), (Vec3(x, y, 0)-Vec3(Width/2, 350, 400)).normalize());
